package Smokes::Mojo::Controller::Site;
use Mojo::Base 'Mojolicious::Controller', -signatures;
use Smokes::Helpers::Sensible;
use Smokes::Tooling::ParseSmokeDB "parse_smoke_report";
...
sub matrix ($self) {
    ...
    my $res = $ua->get('http://perl5.test-smoke.org/api/matrix')->result;

    if ($res->is_success) {
        my $tests = decode_json($res->body);
...
sub submatrix($self) {
    ...
    my $ua  = Mojo::UserAgent->new;
    my $res = $ua->get("http://perl5.test-smoke.org/api/submatrix?test=$test&pversion=$pversion")->result; # TODO: Sanitize

    if ($res->is_success) {
        my $smokes = decode_json($res->body);
