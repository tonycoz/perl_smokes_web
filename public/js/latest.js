...
window.addEventListener("load", (event) => {
    ...
    var host_in = document.getElementById("host");
	...
	for (var tr of reports) {
	    ...
	    if (host != "" && ok) {
		ok = match_regexp(cols[4].textContent, host);
	    }
	    if (arch != "" && ok) {
		ok = match_regexp(cols[5].textContent, arch);
	    }
	    if (cc != "" && ok) {
		var sp = cols[5].getElementsByTagName("span");
		var cc_text = sp.length ? sp[0].title : cols[6].textContent;
		ok = match_regexp(cc_text, cc);
	    }
	    if (from != "" && ok) {
		ok = match_regexp(cols[7].textContent, from);
	    }
    ...
    host_in.addEventListener("input", do_search);
